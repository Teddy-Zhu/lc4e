package com.silentgo.lc4e.util.geetest;
...
public class GeetestLib {
    ...
    private String getFailPreProcessRes() {
        ...
        result.put("gt", this.captchaId);
        result.put("challenge", challenge);
        result.put("new_captcha", this.newFailback);
    ...
    private int registerChallenge() {
        ...
        try {
            ...
            JSONObject jsonObject = (JSONObject) JSON.parse(result_str);
            String return_challenge = (String) jsonObject.get("challenge").get();
    ...
    public int enhencedValidateRequest(String challenge, String validate, String seccode) {
        ...
        JSONObject return_map = (JSONObject) JSON.parse(response);
        String return_seccode = (String) return_map.get("seccode").get();
    ...
    public static String getId(PropKit propKit) {
        return propKit == null ? "" : propKit.getValue("geetest.id");
    }

    public static String getKey(PropKit propKit) {
        return propKit == null ? "" : propKit.getValue("geetest.key");

    }

    public boolean verify(Request request, GeetestRequest geetestRequest) {
        String challenge = geetestRequest.getGeetest_challenge();
        String validate = geetestRequest.getGeetest_validate();
        String seccode = geetestRequest.getGeetest_seccode();

        int gt_server_status_code = (Integer) request.getSession().getAttribute(this.gtServerStatusSessionKey);
        int gtResult;
        if (gt_server_status_code == 1) {
            //gt-server正常，向gt-server进行二次验证

            gtResult = this.enhencedValidateRequest(challenge, validate, seccode, "");
            System.out.println(gtResult);
        } else {
            // gt-server非正常情况下，进行failback模式验证

            System.out.println("failback:use your own server captcha validate");
            gtResult = this.failbackValidateRequest(challenge, validate, seccode);
            System.out.println(gtResult);
        }
        return gtResult == 1;
    }
