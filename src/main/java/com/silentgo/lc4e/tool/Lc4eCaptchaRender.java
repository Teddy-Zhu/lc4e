...
public class Lc4eCaptchaRender implements Render {
    ...
    private String graphics(BufferedImage bufferedImage) {
        ...
        // 画粗线
        for (int i = 0; i < 2; i++) {
            this.drawThickLine(graphics, 0, RandomUtil.number(IMG_HEIGTH) + 1, IMG_WIDTH, RandomUtil.number(IMG_HEIGTH) + 1, 4, getRandColor(100, 200));// 加一道线
        }

        // 从左上到右下加上多道干扰线
        graphics.setColor(getRandColor(160, 200));
        for (int i = 0; i < 80; i++) {
            int x = RandomUtil.number(IMG_WIDTH - 1);
            int y = RandomUtil.number(IMG_HEIGTH - 1);
            int xl = RandomUtil.number(6) + 1;
            int yl = RandomUtil.number(12) + 1;
        ...
        for (int i = 0; i < 80; i++) {
            int x = RandomUtil.number(IMG_WIDTH - 1);
            int y = RandomUtil.number(IMG_HEIGTH - 1);
            int xl = RandomUtil.number(12) + 1;
            int yl = RandomUtil.number(6) + 1;
            graphics.drawLine(x, y, x - xl, y - yl);
        }
        ...
        // 设置绘制字符的字体
        int fontIndex = RandomUtil.number(9);
        graphics.setFont(fontList.get(fontIndex));// mFont

        // 生成随机字符串
        String sRand = "";
        for (int i = 0; i < 4; i++) {
            char tmp = RandomUtil.getAuthCodeChar();
            sRand += tmp;
            // 获取随机颜色
            graphics.setColor(new Color(20 + RandomUtil.number(110), 20 + RandomUtil.number(110), 20 + RandomUtil.number(110)));
            // 在图片上绘制系统生成的随机字符
            graphics.drawString(String.valueOf(tmp), 20 * i + 20, RandomUtil.number(10, 28));
        }
        fontIndex = RandomUtil.number(9);
        ...
        for (int i = 0; i < area; i++) {
            int x = RandomUtil.number(IMG_WIDTH);
            int y = RandomUtil.number(IMG_HEIGTH);
    ...
    private Color getRandColor(int fc, int bc) {
        ...
        int r = fc + RandomUtil.number(bc - fc);
        int g = fc + RandomUtil.number(bc - fc);
        int b = fc + RandomUtil.number(bc - fc);
        // 得到随机颜色
    ...
    private void shearX(Graphics g, int w1, int h1, Color color) {
        int period = RandomUtil.number(2);
        boolean borderGap = true;
        int frames = 1;
        int phase = RandomUtil.number(2);
        for (int i = 0; i < h1; i++) {
    ...
    private void shearY(Graphics g, int w1, int h1, Color color) {
        int period = RandomUtil.number(40) + 10; // 50;
        boolean borderGap = true;
        int frames = 20;
        int phase = 7;
        for (int i = 0; i < w1; i++) {
    ...
    private int getRandomIntColor() {
        int[] rgb = RandomUtil.getRandomRgb();
        int color = 0;
        for (int c : rgb) {
            color = color << 8;
            color = color | c;
